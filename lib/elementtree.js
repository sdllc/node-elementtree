...
ElementTree.prototype.write = function(options) {
  ...
  if (options.method === "text") {
    ...
    var qnames, namespaces, prettyprint, indent_string;
    ...
    if (options.method === "xml") {
      _serialize_xml(function(data) {
        sb.push(data);
      }, this._root, options.encoding, qnames, namespaces, prettyprint, indent_string);
    }
    else {
      /* TODO: html */
  ...
  if (tag === Comment) {
    ...
    if (tag === undefined) {
      ...
      if (text || elem.len()) {
        ...
        if (text) {
          if (newlines) {
            write(Array(indent + 2).join(indent_string));
          }
          write(_escape_cdata(text, encoding));
          if (newlines) {
            write("\n");
