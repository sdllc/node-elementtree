...
ElementTree.prototype.write = function(options) {
  ...
  if (options.method === "text") {
    ...
    var qnames, namespaces, prettyprint;
    var x = _namespaces(this._root, options.encoding, options.default_namespace);
    qnames = x[0];
    namespaces = x[1];
    prettyprint = options['prettyprint'] ? 0 : false;

    if (options.method === "xml") {
      _serialize_xml(function(data) {
        sb.push(data);
      }, this._root, options.encoding, qnames, namespaces, prettyprint);
    }
    else {
      /* TODO: html */
...
function _serialize_xml(write, elem, encoding, qnames, namespaces, indent) {
  var tag = elem.tag;
  var text = elem.text;
  var items;
  var i;
  ...
  if (tag === Comment) {
    ...
    if (tag === undefined) {
      ...
      elem.iter(function(e) {
        _serialize_xml(write, e, encoding, qnames, null, newlines ? indent + 1 : false);
      });
    }
    else {
      write("<" + tag);
